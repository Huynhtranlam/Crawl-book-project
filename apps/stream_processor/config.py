...
class StreamProcessorConfig:
    ...
    def from_env(cls) -> "StreamProcessorConfig":
        ...
        return cls(
            ...
            or "stream-processor",
            kafka_auto_offset_reset=os.getenv("STREAM_PROCESSOR_AUTO_OFFSET_RESET", "earliest").strip()
            or "earliest",
            kafka_poll_timeout_ms=int(os.getenv("STREAM_PROCESSOR_POLL_TIMEOUT_MS", "10000")),
            kafka_batch_size=int(os.getenv("STREAM_PROCESSOR_BATCH_SIZE", "10")),
            postgres_host=postgres_host,
            postgres_port=int(os.getenv("POSTGRES_PORT", "5432")),
            postgres_db=postgres_db,
            postgres_user=postgres_user,
            postgres_password=postgres_password,
            postgres_table=os.getenv("STREAM_PROCESSOR_POSTGRES_TABLE", "products_clean").strip()
            or "products_clean",
            error_output_dir=os.getenv("STREAM_PROCESSOR_ERROR_DIR", "apps/stream_processor/errors").strip()
